# DUPLICATED FROM modules/install/partitions — see TODO
: "${install_device:=}"
: "${boot_size:=}"
: "${swap_size:=}"
: "${root_size:=}"
...
partitions_has_expected_layout() {
  ...
  [[ $(echo "$types" | grep -c "0FC63DAF-8483-4772-8E79-3D69D8477DE4") -eq 2 ]] || return 1 # 2 Linux
  [[ $(echo "$types" | wc -l) -eq 4 ]]                                         # exactly 4 entries
}
