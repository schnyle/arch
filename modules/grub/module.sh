...
bootloader_remove_orphaned_entries() {
  local boot_num uuid boot_device removed
...
configure() {
  ...
  if [[ $grub_bootloader_exists -ne 0 ]] && [[ ! -f /boot/EFI/GRUB/grubx64.efi ]]; then
  ...
  if [[ $grub_bootloader_exists -ne 0 ]] && [[ ! -f /boot/EFI/BOOT/BOOTX64.EFI ]]; then
